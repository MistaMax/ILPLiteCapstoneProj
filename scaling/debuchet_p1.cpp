...
void debuchet_p1(ILPData data)
{
	...
	for (int i = 0; i < m; i++) {
		...
		if (!ind.empty())
		{
			...
			row_inv_sum(i) = sum_abs(1 / data.A.row(i), ind);
			row_multi(i) = pow(row_inv_sum(i) / row_sum(i),.5);
	...
	for (int i = 0; i < n; i++) {
		...
		if (!ind.empty())
		{
			...
			col_inv_sum(i) = sum_abs(1 / data.A.col(i), ind);
			col_multi(i) = pow(col_inv_sum(i) / col_sum(i), .5);
